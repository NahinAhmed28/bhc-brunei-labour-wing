                        ...
                        <tbody>
                            @forelse($userTokenCounts as $userTokenCount)
                                <tr>
                                    ...
                                    <td><a class="text-decoration-none" href="{{ route('tokens.index', ['holder_id' => $userTokenCount->id]) }}">{{ number_format($userTokenCount->held_tokens_count) }}</a></td>
                                    <td class="pe-4">{{ number_format($userTokenCount->created_tokens_count) }}</td>
                                </tr>
                            @empty
                                <tr><td class="text-secondary px-4 py-4" colspan="3">No tokens have been assigned or generated by an active user.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
